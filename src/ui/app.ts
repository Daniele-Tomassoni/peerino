...
function formatTurnLimitMessage(fileSize: number, maxSize: number): string {
    ...
    return `⚠️ File too large for relay\n\n` +
        ...
        `A direct connection could not be established, so the transfer is using the relay. ` +
        `This can happen with some WiFi networks, mobile networks, VPNs, or strict routers.\n\n` +
        `To avoid the relay, try:\n` +
        `• Connecting both devices to a regular WiFi network\n` +
        `• Disabling VPN or proxy restrictions\n` +
        `• Allowing UDP traffic through the router or firewall\n` +
        `• Trying again later if the network is changing\n\n` +
        `Two devices on different WiFi networks may still connect directly.`;
}
