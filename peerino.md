# Peerino — Documento di Sintesi del Progetto
...
### 1.1 Scopo e Dominio
**Peerino** è un'applicazione desktop cross-platform per la **condivisione di file peer-to-peer** (P2P) costruita con **Tauri 2.0** (Rust backend + WebView frontend). L'obiettivo dichiarato nel file [`project-context.md:30`](project-context.md:30) è realizzare un sistema di condivisione file *decentralizzato*, *senza server centrali* e con un'architettura incrementale articolata in sei fasi di sviluppo.
...
- **Sviluppatori** che intendono estendere l'app secondo la roadmap documentata in `project-context.md`.

### 1.3 Stato del Progetto
- **Versione corrente**: `3.2.0` (vedi [`package.json:3`](package.json:3), [`src-tauri/Cargo.toml:3`](src-tauri/Cargo.toml:3), [`src-tauri/tauri.conf.json:4`](src-tauri/tauri.conf.json:4)).
- **Fase completata**: Fase 4 (P2P su Internet) con funzionalità avanzate. Il codice contiene già il sistema di cancellazione robusto (P0–P3) documentato in [`CHECKPOINT.md:1`](CHECKPOINT.md:1).
- **Fase pianificata**: Fase 5 (P2P locale con crediti) e Fase 6 (mobile).
- **Modalità di sviluppo**: il P2P diretto app↔app via PeerJS è implementato ma **nascosto nell'UI** (la UI di connessione manuale è stata rimossa; il motore PeerJS gira *headless* per generare il PeerID necessario ai link web). Vedi [`src/ui/app.ts:902-910`](src/ui/app.ts:902) (commento esplicito) e [`src/ui/index.html`](src/ui/index.html).
- **Tipo di repository**: non è un repository Git (vedi [`CHECKPOINT.md:37`](CHECKPOINT.md:37)).

### 1.4 Caratteristiche Distintive
- **Streaming obbligatorio**: nessun `Vec<u8>` viene passato per i file (regola architetturale esplicita, vedi [`src-tauri/src/commands/register_file.rs:87-113`](src-tauri/src/commands/register_file.rs:87) e [`src-tauri/src/commands/p2p/stream_file.rs:39-63`](src-tauri/src/commands/p2p/stream_file.rs:39)). I file sono gestiti con `tokio::fs` e buffer da 64KB.
- **Tauri Channels** per lo streaming binario ad alta frequenza (`tauri::ipc::Channel<Vec<u8>>`).
- **Capabilities Tauri 2.0** dichiarate in file dedicati ([`src-tauri/capabilities/default.json`](src-tauri/capabilities/default.json)), mai in `tauri.conf.json`.
